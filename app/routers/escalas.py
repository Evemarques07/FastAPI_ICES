...
def create_escala(escala_in: escala.EscalaCreate, db: Session = Depends(database.get_db), user=Depends(get_current_user)):
...
def update_escala(escala_id: int, escala_in: escala.EscalaUpdate, db: Session = Depends(database.get_db), user=Depends(get_current_user)):
...
def delete_escala(escala_id: int, db: Session = Depends(database.get_db), user=Depends(get_current_user)):
    db_escala = db.query(models.escala.Escala).filter_by(id=escala_id).first()
    if not db_escala:
        raise HTTPException(status_code=404, detail="Escala não encontrada")
    db.delete(db_escala)

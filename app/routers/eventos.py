...
def create_evento(evento: evento.EventoCreate, db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
...
def list_eventos(skip: int = 0, limit: int = 20, db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
...
def list_eventos_ativos(db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
    eventos = db.query(models.evento.Evento).filter_by(ativo=True).all()
    return eventos

@router.get("/{evento_id}", response_model=evento.EventoOut)
def get_evento(evento_id: int, db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
...
def update_evento(evento_id: int, evento: evento.EventoUpdate, db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
...
def delete_evento(evento_id: int, db: Session = Depends(database.get_db), user=Depends(get_current_secretario)):
    db_evento = db.query(models.evento.Evento).filter_by(id=evento_id).first()
    if not db_evento:
        raise HTTPException(status_code=404, detail="Evento não encontrado")
    db.delete(db_evento)
